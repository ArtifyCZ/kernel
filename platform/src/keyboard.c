...
static inline uint32_t kbd_next(uint32_t x) { return (x + 1) & (KBD_BUF_SIZE - 1); }
...
char kbd_translate_scancode(uint8_t sc) {
    ...
    if (!is_break(sc) && (sc == 0x2A || sc == 0x36)) { shift_down = true; return 0; }
    if ( is_break(sc) && (make_code(sc) == 0x2A || make_code(sc) == 0x36)) { shift_down = false; return 0; }
...
char keyboard_read_char(void) {
    char scan_code;
    if (!kbd_scancode_pop(&scan_code)) return 0;
    char ascii[256] =
    {
    ...
    return kbd_translate_scancode(scan_code);
}
